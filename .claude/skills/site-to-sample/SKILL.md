...
```bash
node .claude/skills/site-to-sample/scripts/extract-part.mjs "<URL>"
```
...
```bash
node .claude/skills/site-to-sample/scripts/extract-part.mjs "<URL>" "<選ばれたselector>"
```
...
`config/webpack.config.js` の編集は不要（globで自動検出される）。
